...
struct EditNoteModal: View {
    ...
    let note: Note
    @ObservedObject var firebase: Firebase
    var onNoteUpdated: (Note) -> Void // Update to accept a Note

    init(note: Note, firebase: Firebase, onNoteUpdated: @escaping (Note) -> Void) {
        self.note = note
        self.firebase = firebase
        self.onNoteUpdated = onNoteUpdated
        _newTitle = State(initialValue: note.title)
                ...
                Button("Update Note") {
                    ...
                    firebase.updateNoteContentCompletion(note: note, newContent: newContent) { updatedNote in
                        if let updatedNote = updatedNote {
                            // Then update the title
                            firebase.updateNoteTitle(note: updatedNote, newTitle: newTitle) { _ in
                                var finalUpdatedNote = updatedNote
                                finalUpdatedNote.title = newTitle
                                onNoteUpdated(finalUpdatedNote) // Pass the updated note
                                dismiss()
                            }
                        }
                    }
